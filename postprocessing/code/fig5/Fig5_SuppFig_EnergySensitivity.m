...
OUT_PDF = fullfile(RESULTS_DIR, 'SuppFig_EnergySensitivity.pdf');
OUT_FIG = fullfile(RESULTS_DIR, 'SuppFig_EnergySensitivity.fig');
...
% E_threshold = 50;   % kWh, matches paper's ~50 kWh threshold, rounded up
% 
...
E_threshold = 50;   % kWh, consistent with break-even threshold in main analysis
...
for s = 1:n_en
    ...
    E     = samples_en{s}.E_pack_nom;
    fprintf('  %-22s  %8.2f  %8.2f  %8.2f  %7.1f%%  %9.1f%%  %9.1f%%\n', ...
        labels_en{s}, 1000*median(d), 1000*prctile(d,25), 1000*prctile(d,75), ...
        100*mean(d > 0), ...
        100*mean(d(E >= E_threshold) > 0), ...
        100*mean(d(E <  E_threshold) > 0));
...
for s = 1:n_eta
    ...
    E     = samples_eta{s}.E_pack_nom;
    fprintf('  %-22s  %8.2f  %8.2f  %8.2f  %7.1f%%  %9.1f%%  %9.1f%%\n', ...
        labels_eta{s}, 1000*median(d), 1000*prctile(d,25), 1000*prctile(d,75), ...
        100*mean(d > 0), ...
        100*mean(d(E >= E_threshold) > 0), ...
        100*mean(d(E <  E_threshold) > 0));
...
for s = 1:n_en
    d = delta_NPC_en{s};
    text(axEn, s, ylims_shared(2) * 0.92, ...
        sprintf('Median: \n%7.2f EUR\n%7.1f\\%% $>$0', 1000*median(d), 100*mean(d>0)), ...
...
ylabel(axEn, '$\Delta \mathrm{NPC}\ [\mathrm{kEUR}]$', ...
    'Interpreter', 'latex', 'FontName', 'Latin Modern Roman','FontSize', 12);
xlabel(axEn, 'Electricity price [EUR/kWh]', ...
    'Interpreter', 'latex', 'FontName', 'Latin Modern Roman','FontSize', 12);
grid(axEn, 'on'); box(axEn, 'on');
addPanelLabel(axEn, '\textbf{(a)}', -0.15, 1.0, 12);
hold(axEn, 'off');
...
for s = 1:n_eta
    d = delta_NPC_eta{s};
    text(axEta, s, ylims_shared(2) * 0.92, ...
        sprintf('Median: \n%7.2f EUR\n%7.1f\\%% $>$0', 1000*median(d), 100*mean(d>0)), ...
        'HorizontalAlignment', 'center', ...
        'VerticalAlignment',   'top', ...
        'FontSize', 12, ...
...
ylabel(axEta, '$\Delta \mathrm{NPC}\ [\mathrm{kEUR}]$', ...
    'Interpreter', 'latex', 'FontName', 'Latin Modern Roman','FontSize', 12);
xlabel(axEta, 'Energy consumption [kWh/km]', ...
    'Interpreter', 'latex', 'FontName', 'Latin Modern Roman','FontSize', 12);
grid(axEta, 'on'); box(axEta, 'on');
addPanelLabel(axEta, '\textbf{(b)}', -0.15, 1.0, 12);
hold(axEta, 'off');
...
exportgraphics(figCombined, OUT_PDF, 'ContentType', 'image', 'Resolution', 600);
